...
#    for trial in range(max_trials):
    for sample_size in sample_sizes:
        ...
        net = instantiate_cnn2(1, 10, 28, 28)
...
#        net.fit(np.reshape(train_x, ([train_x.shape[0], 28, 28, 1])), train_y, validation_data=(val_x, val_y),
        #net.fit(np.reshape(train_x, ([train_x.shape[0], 28, 28, 1])), train_y, validation_data=(val_x, val_y), callbacks=[early_stopping], batch_size=16, verbose = 1, epochs = 1000, shuffle=True)
        if sample_size < 6000:
