...
class GSFANode(mdp.nodes.SFANode):
    ...
    def _stop_training(self, debug=False, verbose=False, pca_term=0.995, pca_exp=2.0):
        ...
        try:
	        ...
	        print '[svd]'
		U, s, _ = numpy.linalg.svd(self.cov_mtx)
		nonzeros = (s > 1e-7).sum()
		X1 = numpy.dot( U[:, :nonzeros], numpy.diag(1.0/s[:nonzeros]**0.5) )
		X2, s2, _ = numpy.linalg.svd( numpy.dot(X1.T, numpy.dot(self.dcov_mtx,X1)) )
		E = numpy.dot( X1, X2 )
		e = numpy.dot( E.T, numpy.dot(self.dcov_mtx,E) ).diagonal()
